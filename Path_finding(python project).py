...
def best():
            ...
            if len(openSet) > 0:
                ...
                if current == end:
                    ...
                    temp=heurisitic(current, start)
                    print('done', round(current.f))
                    temp=round(temp)
                    start.show((255,8,127),0)
                ...
                for i in range(len(neighbors)):
                    neighbor = neighbors[i]
                    if neighbor not in closedSet and neighbor not in openSet:
...
label2 = Label(window, text='Select Algorithm')
label2.grid(row=3,pady=1)
endBox = Entry(window)
var = IntVar()
...
rb=Radiobutton(window, text = "BFS", 
        value = "1",variable=radio)
rb.grid(row=4,pady=4)
rb=Radiobutton(window, text = "A*", 
        value = "2",variable=radio)
rb.grid(row=5,pady=5)
  

showPath.grid(columnspan=2, row=2)
submit.grid(columnspan=5, row=7)
